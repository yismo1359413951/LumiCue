...
final class AppCoordinator {
  ...
  func applicationDidFinishLaunching() {
    ...
    // 靓相 Shotlit: 只做隐形提词器, 不再弹 LumiCue 欢迎/启动窗
  ...
  func applicationWillTerminate() {
    ...
    LumiCueConfigurationSyncCoordinator.shared.stop()
  ...
  func handleDeepLink(_ url: URL) {
    LumiCueDeepLinkHandler(screenCaptureViewModel: environment.screenCaptureViewModel)
      .handle(url)
  }
  ...
  private func applyUserConfigurationIfNeeded() -> LumiCueConfigurationAutoImportResult {
    let result = LumiCueConfigurationAutoImporter.applyIfNeededOnLaunch()
    let context = [
  ...
  private func startConfigurationSync(after autoImportResult: LumiCueConfigurationAutoImportResult) {
    let coordinator = LumiCueConfigurationSyncCoordinator.shared
    coordinator.start()
  ...
  private func flushConfigurationSyncBeforeTermination() {
    do {
      try LumiCueConfigurationSyncCoordinator.shared.flushPendingSync(reason: .appTerminate)
    } catch {
  ...
  private func presentStartupExperience(
    configurationAutoImportResult: LumiCueConfigurationAutoImportResult
  ) {
  ...
  private func shouldPresentConfigurationAccessOnboarding(
    for result: LumiCueConfigurationAutoImportResult
  ) -> Bool {
