...
struct AppLaunchPolicy {
  ...
  private var allowsInteractiveXCTestHost: Bool {
    environment["LUMICUE_ALLOW_INTERACTIVE_XCTEST_HOST"] == "1"
  }
}
...
class AppDelegate: NSObject, NSApplicationDelegate {
  ...
  func applicationDidFinishLaunching(_ notification: Notification) {
    ...
    // LumiCue 纯提词器模式: 不启动 LumiCue 的状态栏菜单/录屏/截屏那套(那来自 coordinator),
    // 只开提词器窗口。打开即提词器, 无"退出LumiCue"状态栏菜单, ✕ 关闭即退出 app。
  ...
  private func presentSandboxOffMigrationRecoveryAlert(error: Error) -> SandboxOffMigrationRecoveryAction {
    ...
    alert.messageText = "LumiCue could not migrate your existing data."
    alert.informativeText = """
      LumiCue needs to move data from the old sandboxed storage before opening the unsandboxed version.
    ...
    alert.addButton(withTitle: "Quit LumiCue")
    ...
    while true {
      ...
      case .repair:
        do {
          ...
          currentError = error
          note = "Repair did not succeed. You can reset the database after backing up the current files, or quit LumiCue."
        }
      ...
      case .reset:
        ...
        do {
          ...
          case let .failure(error):
            currentError = error
            note = "Reset moved the old database files aside, but LumiCue still could not create a fresh database."
          }
        } catch {
          currentError = error
          note = "Reset failed before LumiCue could create a fresh database."
        }
  ...
  private func presentDatabaseRecoveryAlert(
    ...
    let alert = NSAlert()
    alert.alertStyle = .critical
    alert.messageText = "LumiCue could not open its database."

    var informativeText = """
      LumiCue needs this database for capture history and cloud upload records.
  ...
  private func confirmDatabaseReset(error: Error) -> Bool {
    let alert = NSAlert()
    alert.alertStyle = .critical
    alert.messageText = "Reset LumiCue Database?"
    alert.informativeText = """
      LumiCue will move the current database files into a recovery folder, then create a new empty database.

      This resets capture history and cloud upload history inside LumiCue. Capture files on disk and cloud files are not deleted.
  ...
  // MARK: - Open With (Finder right-click → Open With → LumiCue)

  /// Called when the user opens one or more image files with LumiCue from
  /// Finder's "Open With" submenu, by double-clicking a file whose default
  /// app is LumiCue, or by drag-dropping files onto the app icon in the Dock.
  ///
  ...
  /// Apple Event handler for `lumicue://` deep links keeps working.
