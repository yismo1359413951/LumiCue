//
//  LumiCueDeepLinkHandler.swift
//  LumiCue
//
//  Handles lumicue:// automation URLs for external launchers and workflows.
//
...
struct LumiCueDeepLinkHandler {
  ...
  func handle(_ url: URL) {
    guard let action = LumiCueDeepLinkAction(url: url) else {
...
enum LumiCueDeepLinkAction: Equatable {
  case captureFullscreen
  case captureArea
  case captureApplication
  case captureActiveWindow
  ...
  init?(url: URL) {
    guard url.scheme?.lowercased() == "lumicue" else { return nil }
