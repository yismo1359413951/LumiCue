...
struct AdvancedSettingsView: View {
  ...
  @State private var needsConfigAccess = LumiCueConfigurationService.shared.needsUserSelectedConfigAccess
  ...
  @ObservedObject private var configSyncCoordinator = LumiCueConfigurationSyncCoordinator.shared

  private let service = LumiCueConfigurationService.shared
  ...
  private var effectiveConfigSyncStatus: LumiCueConfigurationSyncCoordinator.Status {
  ...
  private func grantConfigAccess(openAfterGrant: Bool) {
    do {
      guard let grantResult = try LumiCueConfigurationAccessGranting.grantSuggestedConfigAccess(service: service) else {
        return
      }
  ...
  private func issues(for autoImportResult: LumiCueConfigurationAutoImportResult) -> [LumiCueConfigurationIssue] {
    ...
    if autoImportResult.status == .failed, let errorMessage = autoImportResult.errorMessage {
      return [LumiCueConfigurationIssue(severity: .error, message: errorMessage)]
    }
  ...
  private func showGrantNotice(for grantResult: LumiCueConfigurationAccessGrantResult) {
  ...
  private func showImportNotice(for result: LumiCueConfigurationImportResult) {
    showNotice(
  ...
  private func showRestoreNotice(for result: LumiCueConfigurationImportResult) {
  ...
  private func noticeSummary(
    for result: LumiCueConfigurationImportResult,
  ...
  private func noticeStyle(for issues: [LumiCueConfigurationIssue]) -> AppToastStyle {
    ...
    HStack(alignment: .center, spacing: 12) {
      ...
      VStack(alignment: .leading, spacing: 3) {
        ...
        Text(L10n.PreferencesAdvanced.configAccessWarningDescription(
          LumiCueConfigurationService.shared.suggestedConfigDirectoryURL.path
        ))
        .font(.caption)
