...
final class AreaSelectionController: NSObject {
  ...
  /// fires while LumiCue is the active app; on a `.nonactivatingPanel` shown via a global
  ...
  private func startSelectionSession(
    ...
    // Bring LumiCue forward so the overlay's transparent crosshair cursor takes effect immediately.
    ...
    // activating would deactivate/dim them and leave LumiCue frontmost afterward; those keep the
  ...
  private func installManualSelectionMonitorIfNeeded() {
    ...
    // Global monitor receives drag/up even while LumiCue is inactive (the first ⌘⇧4 gesture on a
