...
enum CaptureOutputKind {
  ...
  var defaultTemplate: String {
    switch self {
    case .screenshot:
      return "LumiCue_{datetime}_{ms}"
    case .recording:
      return "LumiCue_Recording_{datetime}"
    }
  }
...
enum CaptureOutputNaming {
  ...
  private static func fallbackName(for kind: CaptureOutputKind, date: Date) -> String {
    switch kind {
    case .screenshot:
      return "LumiCue_\(format(date, style: "yyyy-MM-dd_HH-mm-ss-SSS"))"
    case .recording:
      return "LumiCue_Recording_\(format(date, style: "yyyy-MM-dd_HH-mm-ss"))"
    }
  }
}
