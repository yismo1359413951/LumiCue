...
private let logger = Logger(subsystem: "LumiCue", category: "ScreenCaptureManager")
...
final class ScreenCaptureManager: ObservableObject {
  ...
  private nonisolated static func verifyFileWritten(at url: URL, maxAttempts: Int = 3, delayMs: UInt64 = 50) async -> Bool {
    let logger = Logger(subsystem: "LumiCue", category: "ScreenCaptureManager")
  ...
  private static func captureImageCompat(
    ...
    if #available(macOS 14.0, *) {
      ...
      return try await withCheckedThrowingContinuation { continuation in
        ...
        do {
          try stream.addStreamOutput(handler, type: .screen, sampleHandlerQueue: DispatchQueue(label: "com.yismo.lumicue.screenshot"))
        } catch {
