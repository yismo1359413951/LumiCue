...
enum RecordingAudioCompatibilityExporter {
  ...
  private static func writeNormalizedFile(
    ...
    try await withCheckedThrowingContinuation { continuation in
      let workerQueue = DispatchQueue(label: "com.yismo.lumicue.recording.audio-compatibility", qos: .utility)
      workerQueue.async {
        do {
  ...
  private static func copySamples(
    ...
    for (label, output, input) in outputsAndInputs {
      group.enter()
      let queue = DispatchQueue(label: "com.yismo.lumicue.recording.audio-compatibility.\(label)")
      var didFinish = false
...
final class ScreenRecordingManager: NSObject, ObservableObject {
  ...
  private let videoProcessingQueue = DispatchQueue(
    label: "com.yismo.lumicue.recording.video",
    qos: .userInitiated
  )
  private let audioProcessingQueue = DispatchQueue(
    label: "com.yismo.lumicue.recording.audio",
    qos: .userInteractive
  )
  private let microphoneProcessingQueue = DispatchQueue(
    label: "com.yismo.lumicue.recording.microphone",
    qos: .userInteractive
  )
