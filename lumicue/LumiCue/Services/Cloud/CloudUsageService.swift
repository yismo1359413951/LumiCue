...
private let logger = Logger(subsystem: "LumiCue", category: "CloudUsageService")
...
private actor CloudUsageWorker {
  ...
  private func listAllObjects(
    ...
    repeat {
      var queryString = "list-type=2&prefix=lumicue/&max-keys=1000"
  ...
  private func getLifecycleRuleDays(
    ...
    return LifecycleRuleParser.parseLumiCueExpireDays(from: data)
  }
...
/// Parses lifecycle configuration XML to find the LumiCue auto-expire rule.
final class LifecycleRuleParser: NSObject, XMLParserDelegate {
  ...
  static func parseLumiCueExpireDays(from data: Data) -> Int? {
  ...
  func parser(
    ...
    if insideRule {
      ...
      case "Rule":
        if currentRuleID == "lumicue-auto-expire", let days = currentDays {
          foundDays = days
        }
        insideRule = false
      default:
