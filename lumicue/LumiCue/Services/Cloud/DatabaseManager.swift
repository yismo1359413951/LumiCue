...
private let logger = Logger(subsystem: "LumiCue", category: "DatabaseManager")
...
enum DatabaseInitializationError: LocalizedError {
  ...
  var recoverySuggestion: String? {
    "Try repairing the database, reset it after backing up the existing files, or quit LumiCue."
  }

  var databaseURL: URL {
    switch self {
    case let .directoryCreationFailed(directoryURL, _):
      return directoryURL.appendingPathComponent("lumicue.db")
...
final class DatabaseManager: @unchecked Sendable {
  ...
  private static let databaseFileNames = ["lumicue.db", "lumicue.db-wal", "lumicue.db-shm"]
  ...
  static var defaultDatabaseURL: URL {
    databaseDirectory().appendingPathComponent("lumicue.db")
  }
  ...
  private static func databaseDirectory() -> URL {
      ...
      if isRunningUnderXCTest {
        ...
        return FileManager.default.temporaryDirectory
          .appendingPathComponent("LumiCueTests", isDirectory: true)
    ...
    return appSupport.appendingPathComponent("LumiCue", isDirectory: true)
  }
