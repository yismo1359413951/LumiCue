...
private let logger = Logger(subsystem: "LumiCue", category: "S3CloudProvider")
...
final class S3CloudProvider: CloudProvider {
  ...
  /// LumiCue lifecycle rule identifier
  private static let lifecycleRuleID = "lumicue-auto-expire"
  /// Prefix all LumiCue objects share
  private static let objectPrefix = "lumicue/"
  ...
  func setExpiration(days: Int) async throws {
    ...
    // 2. Build new LumiCue rule (compact XML — R2 is strict about whitespace)
    let lumicueRule = "<Rule><ID>\(Self.lifecycleRuleID)</ID><Filter><Prefix>\(Self.objectPrefix)</Prefix></Filter><Status>Enabled</Status><Expiration><Days>\(days)</Days></Expiration></Rule>"
    existingRules.append(lumicueRule)

    // 3. PUT merged config
    try await putLifecycleConfiguration(rules: existingRules)
    logger.info("Lifecycle rule set: expire lumicue/ objects after \(days) days")
  }
  ...
  func removeExpiration() async throws {
    ...
    logger.info("Lifecycle rule removed for lumicue/ prefix")
  }
  ...
  private func generateObjectKey(fileName: String) -> String {
    ...
    let name = (fileName as NSString).deletingPathExtension
      ...
      .lowercased()
    return "lumicue/\(timestamp)-\(uuid)-\(name).\(ext)"
  }
