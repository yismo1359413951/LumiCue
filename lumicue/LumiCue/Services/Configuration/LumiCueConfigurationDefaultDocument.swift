//
//  LumiCueConfigurationDefaultDocument.swift
//  LumiCue
//
...
@MainActor
enum LumiCueConfigurationDefaultDocument {
  static func toml() -> String {
    var writer = SimpleTOMLWriter()
    writer.root("schema_version", 1)
    writer.root("lumicue_min_version", Bundle.main.infoDictionary?["CFBundleShortVersionString"] as? String ?? "1.20.0")
  ...
  private static func writeCapture(_ writer: inout SimpleTOMLWriter) {
    ...
    writer.value("include_lumicue", false)
  ...
  private static func writeRecording(_ writer: inout SimpleTOMLWriter) {
    ...
    writer.value("color", LumiCueConfigurationColor.hexString(from: MouseHighlightConfiguration.defaultHighlightColor))
  ...
  private static func writeShortcutValues(_ writer: inout SimpleTOMLWriter, shortcut: ShortcutConfig?) {
    ...
    writer.value("key", LumiCueConfigurationShortcutCodec.exportKey(shortcut))
    writer.stringArray("modifiers", LumiCueConfigurationShortcutCodec.exportModifiers(shortcut))
  }
