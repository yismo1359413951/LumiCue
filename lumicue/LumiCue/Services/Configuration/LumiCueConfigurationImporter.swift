//
//  LumiCueConfigurationImporter.swift
//  LumiCue
//
//  Applies validated TOML config to LumiCue stores.
//
...
@MainActor
enum LumiCueConfigurationImporter {
  private struct PreparedImport {
    let issues: [LumiCueConfigurationIssue]
    let mutations: [() -> Void]
  }

  static func validateTOML(_ source: String, defaults: UserDefaults = .standard) -> [LumiCueConfigurationIssue] {
    prepareImport(source, defaults: defaults).issues
  }

  static func importTOML(_ source: String, defaults: UserDefaults = .standard) -> LumiCueConfigurationImportResult {
    ...
    guard !preparedImport.issues.contains(where: { $0.severity == .error }) else {
      return LumiCueConfigurationImportResult(appliedChangeCount: 0, issues: preparedImport.issues)
    }
    ...
    return LumiCueConfigurationImportResult(
  ...
  private static func prepareImport(_ source: String, defaults: UserDefaults) -> PreparedImport {
    ...
    do {
      ...
      return PreparedImport(
        issues: [LumiCueConfigurationIssue(severity: .error, message: error.localizedDescription)],
        mutations: []
      )
    }

    var reader = LumiCueConfigurationReader(document: document)
    var mutations: [() -> Void] = []
  ...
  private static func validateSchema(_ reader: inout LumiCueConfigurationReader) {
    guard let schemaVersion = reader.int("schema_version") else { return }
    if schemaVersion != 1 {
      reader.error("Unsupported schema_version \(schemaVersion). LumiCue currently supports schema_version 1.")
    }
  }

  private static func collectGeneral(
    _ reader: inout LumiCueConfigurationReader,
  ...
  private static func collectCapture(
    ...
    collectBool(&reader, "capture", "screenshot", "include_lumicue", mutations: &mutations) {
  ...
  private static func collectRecording(
    ...
    collectBool(&reader, "recording", "include_lumicue", mutations: &mutations) {
    ...
    if let colorHex = reader.string("recording", "mouse_highlight", "color") {
      guard let color = LumiCueConfigurationColor.color(from: colorHex),
  ...
  private static func expandedPath(_ path: String) -> String {
    LumiCueConfigurationPaths.expandedUserPath(path)
  }
}
