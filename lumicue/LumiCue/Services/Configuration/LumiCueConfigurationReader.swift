//
//  LumiCueConfigurationReader.swift
//  LumiCue
//
...
struct LumiCueConfigurationReader {
  let document: SimpleTOMLDocument
  var issues: [LumiCueConfigurationIssue] = []
  ...
  mutating func error(_ message: String) {
    issues.append(LumiCueConfigurationIssue(severity: .error, message: message))
  }

  mutating func warning(_ message: String) {
    issues.append(LumiCueConfigurationIssue(severity: .warning, message: message))
  }
}
