//
//  LumiCueConfigurationService.swift
//  LumiCue
//
//  Facade for exporting and importing LumiCue TOML configuration files.
//

import Foundation

@MainActor
final class LumiCueConfigurationService {
  static let shared = LumiCueConfigurationService()
  nonisolated private static let managedConfigFileQueue = DispatchQueue(
    label: "com.yismo.lumicue.configuration.managed-file",
    qos: .utility
  )
  ...
  private struct ManagedConfigFileSyncOutcome: Sendable {
    let result: LumiCueConfigurationSyncResult
    let sourceToMarkApplied: String?
  }

  var suggestedConfigURL: URL {
    LumiCueConfigurationPaths.suggestedConfigURL
  }

  var suggestedConfigDirectoryURL: URL {
    LumiCueConfigurationPaths.suggestedConfigDirectoryURL
  }
  ...
  var suggestedConfigRootDirectoryURL: URL {
    LumiCueConfigurationPaths.userHomeDirectory
  }
  ...
  func exportTOML() -> String {
    LumiCueConfigurationExporter.exportTOML()
  }
  ...
  ) -> LumiCueConfigurationSyncDecision {
    if fileSource == currentSource {
      return .alreadyCurrent
    }

    if LumiCueConfigurationAutoImporter.isCurrentFileApplied(fileSource, defaults: defaults) {
  ...
  func importTOML(_ source: String) -> LumiCueConfigurationImportResult {
    LumiCueConfigurationImporter.importTOML(source)
  }

  func `import`(from url: URL) throws -> LumiCueConfigurationImportResult {
  ...
  ) throws -> LumiCueConfigurationImportResult {
    let source = try String(contentsOf: url, encoding: .utf8)
    let validationIssues = LumiCueConfigurationImporter.validateTOML(source)

    guard !validationIssues.contains(where: { $0.severity == .error }) else {
      return LumiCueConfigurationImportResult(appliedChangeCount: 0, issues: validationIssues)
    }
  ...
  func restoreDefaultsReplacingManagedConfig() throws -> LumiCueConfigurationImportResult {
    let source = LumiCueConfigurationDefaultDocument.toml()
    let validationIssues = LumiCueConfigurationImporter.validateTOML(source)
  ...
  func prepareManagedConfigForOpening(at url: URL? = nil) throws -> LumiCueConfigurationSyncResult {
    try syncManagedConfigIfSafe(at: url)
  }

  func syncManagedConfigIfSafe(at url: URL? = nil) throws -> LumiCueConfigurationSyncResult {
    if url == nil && needsUserSelectedConfigAccess {
      return LumiCueConfigurationSyncResult(status: .permissionRequired, fileURL: resolvedConfigFileURL)
    }
  ...
  func syncManagedConfigIfSafeInBackground(at url: URL? = nil) async throws -> LumiCueConfigurationSyncResult {
    if url == nil && needsUserSelectedConfigAccess {
      return LumiCueConfigurationSyncResult(status: .permissionRequired, fileURL: resolvedConfigFileURL)
    }
    ...
    try Self.managedConfigFileQueue.sync {
      ...
      if let expectedFileSignature {
        guard fileManager.fileExists(atPath: targetURL.path) else {
          throw LumiCueConfigurationSyncError.fileChangedSinceConfirmation
        }
        let currentFileSource = try String(contentsOf: targetURL, encoding: .utf8)
        let currentFileSignature = LumiCueConfigurationAutoImporter.contentSignature(for: currentFileSource)
  ...
  private func markCurrentFileAppliedIfLatest(_ source: String, operationID: Int) {
    ...
    LumiCueConfigurationAutoImporter.markCurrentFileApplied(source, defaults: defaults)
  }
  ...
  nonisolated private static func syncManagedConfigFile(
    ...
    let currentSignature = LumiCueConfigurationAutoImporter.contentSignature(for: currentSource)
    guard fileManager.fileExists(atPath: fileURL.path) else {
      try currentSource.write(to: fileURL, atomically: true, encoding: .utf8)
      return ManagedConfigFileSyncOutcome(
        result: LumiCueConfigurationSyncResult(
          status: .synced,
          fileURL: fileURL,
    ...
    let fileSignature = LumiCueConfigurationAutoImporter.contentSignature(for: fileSource)
    if fileSource == currentSource {
      return ManagedConfigFileSyncOutcome(
        result: LumiCueConfigurationSyncResult(
