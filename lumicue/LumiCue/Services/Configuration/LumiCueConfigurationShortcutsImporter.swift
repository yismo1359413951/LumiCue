//
//  LumiCueConfigurationShortcutsImporter.swift
//  LumiCue
//
...
@MainActor
extension LumiCueConfigurationImporter {
  ...
  static func quickAccessSlots(
    from reader: inout LumiCueConfigurationReader
  ...
  private static func collectGlobalShortcut(
    ...
    if let key {
      guard let shortcut = LumiCueConfigurationShortcutCodec.shortcut(
        key: key,
        modifiers: modifiers ?? [],
  ...
  private static func collectOverlayShortcut(
    ...
    guard let key,
          let shortcut = LumiCueConfigurationShortcutCodec.overlayShortcut(
            key: key,
            modifiers: modifiers ?? []
          ) else {
