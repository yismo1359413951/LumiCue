//
//  LumiCueConfigurationSyncCoordinator.swift
//  LumiCue
//
...
@MainActor
final class LumiCueConfigurationSyncCoordinator: ObservableObject {
  ...
  static let shared = LumiCueConfigurationSyncCoordinator()

  @Published private(set) var status: Status = .idle

  private let notificationCenter: NotificationCenter
  private let debounceInterval: TimeInterval
  private let syncManagedConfigIfSafe: @MainActor () throws -> LumiCueConfigurationSyncResult
  private let syncManagedConfigIfSafeInBackground: @MainActor () async throws -> LumiCueConfigurationSyncResult
  ...
  init(
    ...
    self.currentSettingsSignature = {
      LumiCueConfigurationAutoImporter.contentSignature(for: service.exportTOML())
    }
  }
  ...
  convenience init(
    ...
    self.init(
      service: LumiCueConfigurationService.shared,
  ...
  init(
    ...
    syncManagedConfigIfSafe: @escaping @MainActor () throws -> LumiCueConfigurationSyncResult,
    syncManagedConfigIfSafeInBackground: (@MainActor () async throws -> LumiCueConfigurationSyncResult)? = nil,
  ...
  @discardableResult
  func flushPendingSync(reason: Reason) throws -> LumiCueConfigurationSyncResult? {
  ...
  @discardableResult
  func syncNow(reason: Reason = .manual) throws -> LumiCueConfigurationSyncResult {
    cancelPendingSync()
    let sequence = beginSync()
    var syncResult: LumiCueConfigurationSyncResult?
    defer {
  ...
  @discardableResult
  func syncNowInBackground(reason: Reason = .manual) async throws -> LumiCueConfigurationSyncResult {
    cancelPendingSync()
    let sequence = beginSync()
  ...
  private func updateStatus(for result: LumiCueConfigurationSyncResult, sequence: Int) {
  ...
  private func finishSyncing(after result: LumiCueConfigurationSyncResult?, sequence: Int) {
  ...
  private func log(_ result: LumiCueConfigurationSyncResult, reason: Reason) {
    DiagnosticLogger.shared.log(
      .debug,
      .preferences,
