...
//  LumiCue
//
//  Manages temporary capture storage in Application Support/LumiCue/Captures.
...
private let logger = Logger(subsystem: "LumiCue", category: "CaptureStorageManager")
...
final class CaptureStorageManager {
  ...
  private let appSupportFolderName = "LumiCue"
  ...
  /// URL to the captures cache directory: Application Support/LumiCue/Captures
  ...
  func clearCache() async throws -> Int {
    ...
    let deletedPaths = await Task.detached(priority: .utility) { () -> [String] in
      ...
      let backgroundLogger = Logger(subsystem: "LumiCue", category: "CaptureStorageManager")
