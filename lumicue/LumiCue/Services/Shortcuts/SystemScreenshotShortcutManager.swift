...
/// Manages detection and resolution of conflicts between active LumiCue shortcuts
...
final class SystemScreenshotShortcutManager {
  ...
  /// currently-enabled LumiCue fullscreen/area/recording shortcuts.
  ///
  ...
  func hasConflictingSystemShortcuts() -> Bool {
    ...
    for kind in GlobalShortcutKind.allCases where kind.isSystemConflictRelevant {
      ...
      guard let lumicueShortcut = KeyboardShortcutManager.shared.shortcut(for: kind) else { continue }

      if !matchingSystemHotkeys(for: kind, shortcut: lumicueShortcut, in: hotkeys).isEmpty {
        return true
      }
    }
  ...
  /// Return human-readable system shortcut names that currently conflict with a proposed LumiCue shortcut.
  ...
  private func matchingSystemHotkeys(
    ...
    relevantSystemHotkeys(for: kind).filter { hotkeyID in
      ...
      if matches {
        DiagnosticLogger.shared.log(
          .info, .action,
          "System screenshot hotkey \(hotkeyID.rawValue) matches LumiCue \(kind.rawValue) shortcut"
        )
      }
      return matches
    }
