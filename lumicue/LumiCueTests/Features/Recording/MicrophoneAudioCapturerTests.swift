...
final class MicrophoneAudioCapturerTests: XCTestCase {
  ...
  func testMicrophoneAudioCapturerStartStopRealMicrophoneIntegration() throws {
    try XCTSkipUnless(
      ProcessInfo.processInfo.environment["LUMICUE_RUN_MICROPHONE_INTEGRATION"] == "1",
      "Real microphone integration is opt-in. Set LUMICUE_RUN_MICROPHONE_INTEGRATION=1 to run."
    )
    try XCTSkipUnless(
