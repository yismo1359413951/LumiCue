...
final class CaptureOutputNamingTests: XCTestCase {
  ...
  override func setUp() {
    ...
    tempDirectory = FileManager.default.temporaryDirectory
      .appendingPathComponent("LumiCueTests_CaptureOutputNaming_\(UUID().uuidString)", isDirectory: true)
  ...
  func testResolveBaseName_withNilCustomName_fallsBackToTemplate() {
    ...
    // Default template: "LumiCue_{datetime}_{ms}"
    XCTAssertTrue(result.hasPrefix("LumiCue_"), "Expected template-based name, got: \(result)")
  ...
  func testResolveBaseName_withWhitespaceOnlyCustomName_fallsBackToTemplate() {
    ...
    XCTAssertTrue(result.hasPrefix("LumiCue_"))
  }
  ...
  func testResolveTemplateBaseName_returnsPreviewSubpath() {
    let result = CaptureOutputNaming.resolveTemplateBaseName(
      "Shots/{timestamp}/LumiCue_{ms}",
      kind: .screenshot,
      date: fixedDate
    )

    XCTAssertEqual(result, "Shots/\(Int(fixedDate.timeIntervalSince1970))/LumiCue_123")
  }
  ...
  func testCaptureOutputKind_defaultTemplates() {
    XCTAssertEqual(CaptureOutputKind.screenshot.defaultTemplate, "LumiCue_{datetime}_{ms}")
    XCTAssertEqual(CaptureOutputKind.recording.defaultTemplate, "LumiCue_Recording_{datetime}")
  }
  ...
  func testResolveBaseName_invalidTemplate_usesFallbackName() {
    ...
    // Fallback format: "LumiCue_{yyyy-MM-dd_HH-mm-ss-SSS}"
    XCTAssertTrue(result.hasPrefix("LumiCue_"), "Expected fallback name, got: \(result)")
  }
}
