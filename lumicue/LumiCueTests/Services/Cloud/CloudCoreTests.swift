...
final class CloudCoreTests: XCTestCase {
  ...
  func testAWSV4SignerSign_addsRequiredHeadersAndAuthorizationScope() throws {
    var request = URLRequest(url: URL(string: "https://example-bucket.s3.us-east-1.amazonaws.com/lumicue/test.png?acl=&z=last&a=first")!)
  ...
  func testAWSV4SignerPresignURL_includesExpectedQueryItems() throws {
    let presignedURL = try AWSV4Signer.presignURL(
      url: URL(string: "https://example-bucket.s3.us-east-1.amazonaws.com/lumicue/test.png")!,
      accessKey: "AKIATEST",
      secretKey: "SECRET",
      region: "us-east-1",
      expireSeconds: 900
    ...
    XCTAssertEqual(components.path, "/lumicue/test.png")
  ...
  func testCloudCredentialTransferSuggestedArchiveFileName_sanitizesBucket() {
    ...
    XCTAssertEqual(
      ...
      "lumicue-cloud-cloudflare-r2-My-Bucket-2026.lumicuecloud"
    )
  }
  ...
  func testLifecycleXMLParser_extractsRuleBlocksInOrder() {
    let xml = """
      <LifecycleConfiguration>
        <Rule><ID>lumicue-expire</ID><Status>Enabled</Status></Rule>
    ...
    XCTAssertTrue(rules[0].contains("<ID>lumicue-expire</ID>"))
  ...
  func testS3CloudProvider_delete_sendsDELETEToObjectURL() async throws {
    ...
    try await provider.delete(key: "lumicue/test.png")
  ...
  func testS3CloudProvider_setExpiration_403getAnd200put_putsFreshRule() async throws {
    ...
    // Verify body contains the lumicue expiration rule
    if let bodyData = putRequest?.httpBody,
       let body = String(data: bodyData, encoding: .utf8) {
      XCTAssertTrue(body.contains("lumicue-auto-expire"))
