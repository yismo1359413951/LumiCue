//
//  LumiCueConfigurationServiceTests.swift
//  LumiCueTests
//
...
@MainActor
final class LumiCueConfigurationServiceTests: XCTestCase {
  func testConfigFileURLAppendsConfigTomlToSelectedDirectory() {
    let directory = URL(fileURLWithPath: "/Users/example/.config/lumicue", isDirectory: true)

    let url = LumiCueConfigurationService.shared.configFileURL(inDirectory: directory)
  ...
  func testSuggestedConfigDirectoryMatchingUsesCanonicalPath() {
    let expectedDirectory = LumiCueConfigurationPaths.suggestedConfigDirectoryURL

    XCTAssertTrue(LumiCueConfigurationService.shared.isSuggestedConfigDirectory(expectedDirectory))
    XCTAssertFalse(
      LumiCueConfigurationService.shared.isSuggestedConfigDirectory(
  ...
  func testSuggestedConfigParentDirectoryMatchingUsesCanonicalPath() {
    let expectedParentDirectory = LumiCueConfigurationPaths.suggestedConfigDirectoryURL
      .deletingLastPathComponent()

    XCTAssertTrue(LumiCueConfigurationService.shared.isSuggestedConfigParentDirectory(expectedParentDirectory))
    XCTAssertFalse(
      LumiCueConfigurationService.shared.isSuggestedConfigParentDirectory(
        expectedParentDirectory.appendingPathComponent("lumicue")
      )
    )
  }

  func testSuggestedConfigRootDirectoryMatchingUsesCanonicalPath() {
    let expectedRootDirectory = LumiCueConfigurationPaths.userHomeDirectory

    XCTAssertTrue(LumiCueConfigurationService.shared.isSuggestedConfigRootDirectory(expectedRootDirectory))
    XCTAssertFalse(
      LumiCueConfigurationService.shared.isSuggestedConfigRootDirectory(
  ...
  func testEnsureConfigExistsCreatesParentDirectoryAndFile() throws {
    ...
    let url = LumiCueConfigurationPaths.suggestedConfigURL(homeDirectory: homeDirectory)

    let returnedURL = try LumiCueConfigurationService.shared.ensureConfigExists(at: url)
  ...
  func testExportIncludesQuickAccessTwoFingerSwipeSetting() throws {
    ...
    let source = LumiCueConfigurationExporter.exportTOML(defaults: UserDefaultsFactory.make())
    let document = try SimpleTOMLParser.parse(source)
  ...
  func testEnsureConfigExistsDoesNotOverwriteExistingFile() throws {
    ...
    try LumiCueConfigurationService.shared.ensureConfigExists(at: url)
  ...
  func testImportBackupReplacingManagedConfigWritesSelectedTomlToManagedFile() throws {
    ...
    let result = try LumiCueConfigurationService.shared.importBackupReplacingManagedConfig(
      from: backupURL,
  ...
  func testSyncDecisionAlreadyCurrentWhenSourcesMatch() {
    ...
    let decision = LumiCueConfigurationService.syncDecision(
      fileSource: source,
  ...
  func testSyncDecisionAutoSyncsWhenFileMatchesLastAppliedSignature() {
    ...
    LumiCueConfigurationAutoImporter.markCurrentFileApplied(fileSource, defaults: defaults)
  ...
  func testPrepareManagedConfigForOpeningCreatesMissingFileFromCurrentSettings() throws {
    try withRestoredLastAppliedSignature {
      ...
      let result = try LumiCueConfigurationService.shared.prepareManagedConfigForOpening(at: managedURL)

      XCTAssertEqual(result.status, .synced)
      XCTAssertTrue(FileManager.default.fileExists(atPath: managedURL.path))
      let source = try String(contentsOf: managedURL, encoding: .utf8)
      XCTAssertTrue(LumiCueConfigurationAutoImporter.isCurrentFileApplied(source))
    }
  }
  ...
  func testPrepareManagedConfigForOpeningAutoSyncsStaleAppOwnedFile() throws {
    try withRestoredLastAppliedSignature {
      ...
      LumiCueConfigurationAutoImporter.markCurrentFileApplied(staleSource)
      ...
      XCTAssertTrue(LumiCueConfigurationAutoImporter.isCurrentFileApplied(syncedSource))
    }
  }
  ...
  func testSyncManagedConfigToCurrentSettingsOverwritesAfterConfirmation() throws {
    try withRestoredLastAppliedSignature {
      ...
      try LumiCueConfigurationService.shared.syncManagedConfigToCurrentSettings(at: managedURL)
      let syncedSource = try String(contentsOf: managedURL, encoding: .utf8)
  ...
  func testSyncManagedConfigToCurrentSettingsIfUnchangedOverwritesApprovedFile() throws {
    try withRestoredLastAppliedSignature {
      ...
      let result = try LumiCueConfigurationService.shared.prepareManagedConfigForOpening(at: managedURL)
      try LumiCueConfigurationService.shared.syncManagedConfigToCurrentSettingsIfUnchanged(
        at: managedURL,
  ...
  func testSyncManagedConfigToCurrentSettingsIfUnchangedDoesNotOverwriteChangedFile() throws {
    try withRestoredLastAppliedSignature {
      ...
      XCTAssertThrowsError(
        ...
        XCTAssertTrue(error is LumiCueConfigurationSyncError)
      }
  ...
  private func temporaryHomeDirectory() -> URL {
    FileManager.default.temporaryDirectory
      .appendingPathComponent("lumicue-config-service-\(UUID().uuidString)", isDirectory: true)
  }
