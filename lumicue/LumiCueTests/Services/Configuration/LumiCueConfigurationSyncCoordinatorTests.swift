//
//  LumiCueConfigurationSyncCoordinatorTests.swift
//  LumiCueTests
//
...
@MainActor
final class LumiCueConfigurationSyncCoordinatorTests: XCTestCase {
  func testScheduleSyncDebouncesRepeatedRequests() async throws {
    ...
    let coordinator = makeCoordinator(
      ...
      sync: {
        syncCount += 1
        return LumiCueConfigurationSyncResult(
          status: .synced,
          fileURL: fileURL,
  ...
  func testSyncNowReportsConflictWithoutConfirmedOverwrite() async throws {
    ...
    let coordinator = makeCoordinator(
      sync: {
        syncCount += 1
        return LumiCueConfigurationSyncResult(status: .needsConfirmation, fileURL: fileURL)
      },
  ...
  func testPermissionRequiredStatusIsExposed() async throws {
    ...
    let coordinator = makeCoordinator(
      sync: {
        LumiCueConfigurationSyncResult(status: .permissionRequired, fileURL: fileURL)
      }
    )
    defer { coordinator.stop() }
  ...
  private func makeCoordinator(
    ...
    sync: @escaping @MainActor () throws -> LumiCueConfigurationSyncResult = {
      LumiCueConfigurationSyncResult(status: .alreadyCurrent, fileURL: URL(fileURLWithPath: "/tmp/config.toml"))
    },
    backgroundSync: (@MainActor () async throws -> LumiCueConfigurationSyncResult)? = nil,
  ...
  ) -> LumiCueConfigurationSyncCoordinator {
    LumiCueConfigurationSyncCoordinator(
      notificationCenter: notificationCenter,
  ...
  private func temporaryConfigURL() -> URL {
    FileManager.default.temporaryDirectory
      .appendingPathComponent("lumicue-config-sync-\(UUID().uuidString).toml")
  }
}
