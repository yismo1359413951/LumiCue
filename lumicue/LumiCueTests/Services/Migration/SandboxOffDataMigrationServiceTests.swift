...
final class SandboxOffDataMigrationServiceTests: XCTestCase {
  ...
  override func setUpWithError() throws {
    ...
    rootDirectory = FileManager.default.temporaryDirectory
      .appendingPathComponent("LumiCueTests_SandboxOffMigration_\(UUID().uuidString)", isDirectory: true)
    ...
    bundleIdentifier = "com.yismo.lumicue.tests.\(UUID().uuidString)"
  ...
  func testRunIfNeeded_migratesSandboxedApplicationSupportPreferencesAndLogsOnce() throws {
    ...
    let sourceAppSupport = sourceData
      .appendingPathComponent("Library/Application Support/LumiCue", isDirectory: true)
    let sourceLogs = sourceData
      .appendingPathComponent("Library/Logs/LumiCue", isDirectory: true)
    ...
    try Data("database".utf8).write(to: sourceAppSupport.appendingPathComponent("lumicue.db"))
    try Data("capture".utf8).write(to: sourceAppSupport.appendingPathComponent("Captures/capture.png"))
    try Data("log".utf8).write(to: sourceLogs.appendingPathComponent("lumicue_2026-06-21.txt"))
    XCTAssertTrue(
      ([
    ...
    XCTAssertTrue(FileManager.default.fileExists(atPath: destinationAppSupport().appendingPathComponent("lumicue.db").path))
    XCTAssertTrue(
    ...
    XCTAssertTrue(
      FileManager.default.fileExists(
        atPath: libraryDirectory.appendingPathComponent("Logs/LumiCue/lumicue_2026-06-21.txt").path
      )
    )
    XCTAssertTrue(
      FileManager.default.fileExists(
  ...
  func testRunIfNeeded_preservesExistingUnsandboxedFilesAndPreferences() throws {
    ...
    try Data("sandbox database".utf8).write(to: sourceAppSupport.appendingPathComponent("lumicue.db"))
    ...
    try Data("current database".utf8).write(to: destinationAppSupport().appendingPathComponent("lumicue.db"))
    ...
    XCTAssertEqual(
      try String(contentsOf: destinationAppSupport().appendingPathComponent("lumicue.db")),
      "current database"
    )
    XCTAssertEqual(
  ...
  private func destinationAppSupport() -> URL {
    applicationSupportDirectory.appendingPathComponent("LumiCue", isDirectory: true)
  }
}
