...
#
# Prerequisites:
#   - Self-signed cert "LumiCue Self-Signed" in keychain
...
CERT_NAME="LumiCue Self-Signed"
ENTITLEMENTS="$PROJECT_DIR/LumiCue/LumiCue.entitlements"
INSTALL_PATH="/Applications/LumiCue.app"
SERVER_PORT=8089
...
build_archive() {
  local archive_path="$TEST_DIR/archive/LumiCue.xcarchive"
  ...
  if ! xcodebuild archive \
    -project "$PROJECT_DIR/LumiCue.xcodeproj" \
    -scheme LumiCue \
...
prepare_version() {
  ...
  local archive_path="$TEST_DIR/archive/LumiCue.xcarchive"
  local app_path="$TEST_DIR/$label/LumiCue.app"
  ...
  ditto "$archive_path/Products/Applications/LumiCue.app" "$app_path"
...
install_v1() {
  ...
  killall LumiCue 2>/dev/null || true
  sleep 1
  rm -rf "$INSTALL_PATH"
  ditto "$TEST_DIR/v1/LumiCue.app" "$INSTALL_PATH"
  echo "  ✅ v1 installed"
}

create_dmg() {
  local dmg_path="$TEST_DIR/server/LumiCue-test.dmg"
  ...
  hdiutil create \
    -volname "LumiCue" \
    -srcfolder "$TEST_DIR/v2/LumiCue.app" \
    -ov -format UDZO \
    "$dmg_path" \
    > /dev/null 2>&1
...
<rss version="2.0" xmlns:sparkle="http://www.andymatuschak.org/xml-namespaces/sparkle" xmlns:dc="http://purl.org/dc/elements/1.1/">
  <channel>
    <title>LumiCue Test Updates</title>
    ...
    <item>
      ...
      <enclosure
        url="http://localhost:${SERVER_PORT}/LumiCue-test.dmg"
...
start_server() {
  ...
  if kill -0 $SERVER_PID 2>/dev/null; then
    ...
    echo "  → DMG:     http://localhost:$SERVER_PORT/LumiCue-test.dmg"
  else
...
run_test() {
  ...
  echo "║                                                          ║"
  echo "║  Steps:                                                  ║"
  echo "║  1. Open LumiCue from /Applications                      ║"
...
case "$cmd" in
  ...
  clean)
    ...
    echo ""
    echo "Note: /Applications/LumiCue.app was NOT removed."
