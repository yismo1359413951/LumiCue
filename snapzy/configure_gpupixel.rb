#!/usr/bin/env ruby
# 用 xcodeproj 安全地把 GpuPixel framework 链接进 Snapzy target。
require 'xcodeproj'

proj_path = 'Snapzy.xcodeproj'
proj = Xcodeproj::Project.open(proj_path)
target = proj.targets.find { |t| t.name == 'Snapzy' }
abort('找不到 Snapzy target') unless target

# 1. framework 文件引用(放 Frameworks 组)
fw_rel = 'Snapzy/ThirdParty/GpuPixel/gpupixel.framework'
...
target.build_configurations.each do |c|
  ...
  fsp << '$(SRCROOT)/Snapzy/ThirdParty/GpuPixel' unless fsp.include?('$(SRCROOT)/Snapzy/ThirdParty/GpuPixel')
  s['FRAMEWORK_SEARCH_PATHS'] = fsp

  hsp = Array(s['HEADER_SEARCH_PATHS'] || ['$(inherited)'])
  inc = '$(SRCROOT)/Snapzy/ThirdParty/GpuPixel/include'
  hsp << inc unless hsp.include?(inc)
  s['HEADER_SEARCH_PATHS'] = hsp

  s['SWIFT_OBJC_BRIDGING_HEADER'] = 'Snapzy/Snapzy-Bridging-Header.h'
