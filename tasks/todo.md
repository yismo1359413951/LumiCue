...
- [x] 🔴 收起态(2-3行横条)也要控制条放右上角: 后退/暂停/速度/展开(方便暂停后退) ← 2026-06-25
- [ ] 🔴 速度要可见数值(0.1/0.2…)+展开态也放快慢 ← 2026-06-25
- [x] 🟡 高级光束(焦点行呼吸光晕)+复古像素进度——v1做了
- [ ] 🔴🔴 念稿时文字一直在视线里(最核心): 上下行柔和可读·读过的别秒消·没读的提前可见 → 把3D压暗(minOpacity0.16)/虚化调柔和 ← 2026-06-25
- [ ] 🔴 光束再强一点(她没看出区别)
- [ ] 🔴 进度条要"Vibe Island眼前一亮"感 → 像素没味道, 换发光彗星头(亮点+柔光拖尾+脉冲)
- [x] 粘贴纯文本带标点(⌘V走.string已实现)
- [ ] 🔴 粘贴去空行/去格式: 文字紧凑连续一个字挨一个字, 标点留, 删多余空行/首尾空格 ← 2026-06-25
- [ ] 🔴 清空后直接⌘V当场显示(不用点完成), 清后留"⌘V粘贴"提示回显示态 ← 2026-06-25 UX
- [ ] 🟡 彗星头加大加亮(她看不见); 诚实: 新颖靠灵动岛形态+玻璃+光,非进度条小元素
- [ ] ⚪ 🅿️**logo 暂搁**(2026-06-25 用户拍板放下专心做产品): 手搓CSS/SVG做不出"形象/角色"图标, 本机无AI出图key; 以后用 baoyu-imagine(需配key)或免费AI出图站做精致紫色形象图标。试过的烂版在 logo/ 目录
- [ ] 🟡 游戏化="高级光效仪式感"(焦点呼吸发光/念过边流光/进度脉冲)，非具象游戏角色(幼稚)非代码块(小众)——基础框验好再加
  - 用户问"俄罗斯方块那种感觉"→ 我建议不做字面玩法(抢注意+幼稚)，可取复古像素味(进度像素格)呼应Vibe Island 8-bit，待用户定
